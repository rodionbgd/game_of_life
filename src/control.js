...
slider.addEventListener("input", () => {
    ...
    if (Number(slider.value) > 50) {
        tooltip.style.setProperty("left", `calc(${slider.value}%  - ${(Number(slider.value) / 100) * thumbWidth / 2 + tooltip.clientWidth / 2}px)`);
    } else {
        tooltip.style.setProperty("left", `calc(${slider.value}%  - ${tooltip.clientWidth / 2 - (0.5-(Number(slider.value) / 50)) * thumbWidth / 2}px)`);
    }
});